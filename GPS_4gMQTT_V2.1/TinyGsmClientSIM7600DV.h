...
class TinyGsmSim7600 : public TinyGsmModem<TinyGsmSim7600>,
 ...
 protected:
  ...
  bool getGPSImpl(float* lat, float* lon, float* speed = 0, float* cordN = 0,
                  float* hdop = 0, int* hour = 0, int* minute = 0, float* alt = 0,
                  int* vsat = 0, int* usat = 0,  int* year = 0,
                  int* month = 0, int* day = 0, int* second = 0) {
    ...
    if (fixMode == 2 || fixMode == 3) {
      ...
      cog = streamGetFloatBefore(',');                // Course Over Ground. Degrees.
      streamSkipUntil(',');  // After set, will report GPS every x seconds
      ihdop = streamGetFloatBefore(',');  // Position Dilution Of Precision
      ihdop  = streamSkipUntil(',');   // Horizontal Dilution Of Precision
      ...
      if (lon != NULL)
        *lon = (floor(ilon / 100) + fmod(ilon, 100.) / 60) *
            (east == 'E' ? 1 : -1);
      if (speed != NULL) *speed = ispeed * 1.852; //multiplicado por 1.825 para covertir de nudos a km/h
      if (alt != NULL) *alt = ialt;
      if (vsat != NULL) *vsat = ivsat;
      if (usat != NULL) *usat = iusat;
      if (hdop != NULL) *hdop = ihdop;
...
#endif
                      GsmConstStr r5 = NULL) {
    return waitResponse(500, r1, r2, r3, r4, r5);
  }
