...
*/
/**************************************************************************/
PN532DV::PN532DV(uint8_t clk, uint8_t miso, uint8_t mosi,
                 uint8_t ss)
{
  _cs = ss;
  spi_dev = new SPIDeviceDV(ss, clk, miso, mosi, 1000000,
...
*/
/**************************************************************************/
PN532DV::PN532DV(uint8_t ss, SPIClass *theSPI)
{
  _cs = ss;
  spi_dev = new SPIDeviceDV(ss, 1000000, SPI_BITORDER_LSBFIRST,
                            SPI_MODE0, theSPI);
}
...
*/
/**************************************************************************/
PN532DV::PN532DV(uint8_t reset, HardwareSerial *theSer)
    : _reset(reset)
{
...
*/
/**************************************************************************/
void PN532DV::PrintHex(const byte *data, const uint32_t numBytes)
{
  uint32_t szPos;
...
*/
/**************************************************************************/
void PN532DV::PrintHexChar(const byte *data, const uint32_t numBytes)
{
  uint32_t szPos;
  for (szPos = 0; szPos < numBytes; szPos++)
  {
...
*/
/**************************************************************************/
uint32_t PN532DV::getFirmwareVersion(void)
{
  uint32_t response;
...
bool PN532DV::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen,
                                  uint16_t timeout)
{
  ...
  uint8_t SLOWDOWN = 0;
  if (spi_dev) // SPI and I2C need 1ms slow for page reads
    SLOWDOWN = 1;
...
*/
/**************************************************************************/
bool PN532DV::writeGPIO(uint8_t pinstate)
{
  // uint8_t errorbit;
...
*/
/**************************************************************************/
bool PN532DV::setPassiveActivationRetries(uint8_t maxRetries)
{
...
*/
/**************************************************************************/
bool PN532DV::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid,
...
*/
/**************************************************************************/
bool PN532DV::startPassiveTargetIDDetection(uint8_t cardbaudrate)
{
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
  pn532_packetbuffer[1] = 1; // max 1 cards at once (we can set this to 2 later)
  pn532_packetbuffer[2] = cardbaudrate;
...
*/
/**************************************************************************/
bool PN532DV::readDetectedPassiveTargetID(uint8_t *uid,
                                          uint8_t *uidLength)
{
  // read data packet
...
*/
/**************************************************************************/
bool PN532DV::inDataExchange(uint8_t *send, uint8_t sendLength,
                             uint8_t *response,
...
*/
/**************************************************************************/
bool PN532DV::inListPassiveTarget()
{
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
...
*/
/**************************************************************************/
bool PN532DV::mifareclassic_IsFirstBlock(uint32_t uiBlock)
{
...
*/
/**************************************************************************/
bool PN532DV::mifareclassic_IsTrailerBlock(uint32_t uiBlock)
{
  // Test if we are in the small or big sectors
  if (uiBlock < 128)
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareclassic_AuthenticateBlock(uint8_t *uid,
                                                 uint8_t uidLen,
                                                 ...
                                                 uint8_t *keyData)
{
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
    ...
    PN532DV::PrintHexChar(pn532_packetbuffer, 12);
#endif
    return 0;
  }
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareclassic_ReadDataBlock(uint8_t blockNumber,
                                             uint8_t *data)
{
#ifdef MIFAREDEBUG
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
    ...
    PN532DV::PrintHexChar(pn532_packetbuffer, 26);
#endif
    return 0;
  }
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareclassic_WriteDataBlock(uint8_t blockNumber,
                                              uint8_t *data)
{
#ifdef MIFAREDEBUG
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareclassic_FormatNDEF(void)
{
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareclassic_WriteNDEFURI(uint8_t sectorNumber,
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareultralight_ReadPage(uint8_t page,
                                           uint8_t *buffer)
{
  if (page >= 64)
  {
...
*/
/**************************************************************************/
uint8_t PN532DV::mifareultralight_WritePage(uint8_t page,
                                            uint8_t *data)
{
...
*/
/**************************************************************************/
uint8_t PN532DV::ntag2xx_ReadPage(uint8_t page, uint8_t *buffer)
{
...
*/
/**************************************************************************/
uint8_t PN532DV::ntag2xx_WritePage(uint8_t page, uint8_t *data)
{
  // TAG Type       PAGES   USER START    USER STOP
  // --------       -----   ----------    ---------
  // NTAG 203       42      4             39
...
*/
/**************************************************************************/
uint8_t PN532DV::ntag2xx_WriteNDEFURI(uint8_t uriIdentifier, char *url,
                                      uint8_t dataLen)
{
...
*/
/**************************************************************************/
bool PN532DV::waitready(uint16_t timeout)
{
  uint16_t timer = 0;
  while (!isready())
  {
...
*/
/**************************************************************************/
void PN532DV::readdata(uint8_t *buff, uint8_t n)
{
  if (spi_dev)
  {
    // SPI read
...
*/
/**************************************************************************/
uint8_t PN532DV::getDataTarget(uint8_t *cmd, uint8_t *cmdlen)
{
  uint8_t length;
...
*/
/**************************************************************************/
uint8_t PN532DV::setDataTarget(uint8_t *cmd, uint8_t cmdlen)
{
  uint8_t length;
  // cmd1[0] = 0x8E; Must!
...
*/
/**************************************************************************/
void PN532DV::writecommand(uint8_t *cmd, uint8_t cmdlen)
{
  if (spi_dev)
  {
    // SPI command write.
...
// Function to process the UID and convert it to a decimal value
uint64_t PN532DV::processUID(uint8_t *uid, uint8_t uidLength)
{
  uint64_t concatenatedUID = 0;
  for (uint8_t i = 0; i < uidLength; i++)
  {
    concatenatedUID <<= 8;     // Shift left by 8 bits to make space for the next byte
    concatenatedUID |= uid[i]; // Add the next byte
  }

  uint64_t invertedUID = 0;
  for (uint8_t i = 0; i < uidLength; i++)
  {
    invertedUID <<= 8;                       // Shift left by 8 bits to make space for the next byte
    invertedUID |= (concatenatedUID & 0xFF); // Add the lowest byte of concatenatedUID
    concatenatedUID >>= 8;                   // Shift right to remove the byte we just added
  }

  return invertedUID; // Return the inverted UID as a decimal value
}
