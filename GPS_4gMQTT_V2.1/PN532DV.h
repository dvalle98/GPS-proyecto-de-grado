...
/**
 * @brief Class for working with PN532 NFC/RFID breakout boards.
 */
class PN532DV {
public:
  PN532DV(uint8_t clk, uint8_t miso, uint8_t mosi,
                 uint8_t ss);                          // Software SPI
  PN532DV(uint8_t ss, SPIClass *theSPI = &SPI); // Hardware SPI 
  PN532DV(uint8_t reset, HardwareSerial *theSer); // Hardware UART
  bool begin(void);
  ...
  uint64_t processUID(uint8_t* uid, uint8_t uidLength);
